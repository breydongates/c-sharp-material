 ...
 *
 * @param {number} first the first parameter to multiply
 * @param {number} second the second parameter to multiply
 */
...
function sumAllNumbersUsingReduce(numbersToSum) {
  ...
  /*
  const isFlagSet = true;

  return isFlagSet ? 'Flag Was Set' : 'Flag was not set';
  */
 ...
 */
function filterArray(inputArray, filterFunc) {
  //return inputArray.filter(element => filterFunc(element));
  return inputArray.filter(filterFunc);
}
...
const mathHelpers = {
  ...
  multiplyByMultiplier: (num, multiplier) => num * multiplier,
