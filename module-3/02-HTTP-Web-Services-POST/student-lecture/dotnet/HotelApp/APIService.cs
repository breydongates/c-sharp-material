   ...
   public class APIService
    {
        ...
        public Reservation AddReservation(Reservation newReservation)
        {
            ...
            if (response.ResponseStatus != ResponseStatus.Completed)
            {
                Console.WriteLine("Could not add a list of reservations");
                return null;

            }
            if (!response.IsSuccessful)
            {
                Console.WriteLine("Encountered an error adding reservations" + response.ErrorMessage + " (" + response.StatusCode + ") ");
                return null;
            }
            return response.Data;
        }
        ...
        public Reservation UpdateReservation(Reservation reservationToUpdate)
        {
            ...
            if (response.ResponseStatus != ResponseStatus.Completed)
            {
                Console.WriteLine("Could not update a list of reservations");
                return null;

            }
            if (!response.IsSuccessful)
            {
                Console.WriteLine("Encountered an error updating reservations" + response.ErrorMessage + " (" + response.StatusCode + ") ");
                return null;
            }
            return response.Data;
        }
        ...
        public bool DeleteReservation(int reservationId)
        {
            ...
            if (response.ResponseStatus != ResponseStatus.Completed)
            {
                Console.WriteLine("Could not delete a list of reservations");

                return false;
            }
            if (!response.IsSuccessful)
            {
                Console.WriteLine("Encountered an error deleting reservations" + response.ErrorMessage + " (" + response.StatusCode + ") ");
                return false;
            }
            return true;   
        }
