...
Your goal is to create classes for a paint calculator. The calculator works by having a customer input dimensions of walls. Then, the calculator lets them know how many gallons of paint they need to purchase. You need to implement the underlying `Wall` classes that make the calculations possible.
...
To allow polymorphism and reuse common code, an abstract `Wall` class needs to be defined that all other walls inherit from. But it doesn't include any dimensions of its own and, therefore, can't be used in the application as a wall.
